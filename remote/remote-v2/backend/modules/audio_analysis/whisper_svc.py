...
class WhisperService:
    ...
    def __init__(
        ...
        # 从配置读取计算类型
        if compute_type == "float16":  # 如果使用默认值，则从配置读取
            compute_type = getattr(config, 'asr_compute_type', 'float32')
            # CPU 不支持 float16，自动降级为 float32
            if device == "cpu" and compute_type == "float16":
                compute_type = "float32"
                logger.warning("CPU 不支持 float16，已自动切换为 float32")
