...
class DiarizationEngine:
    ...
    def process(
        ...
        # 追加详细日志到现有日志文件
        if log_path:
            self._append_diarization_log(corrected_aligned_words, final_transcript, log_path)
    ...
    def _apply_smoothing(
        ...
        for i in range(1, len(smoothed_words) - 1):
            ...
            if curr_word.get('correction') in ['boundary_corrected_voiceprint', 'boundary_corrected_time']:
    ...
    def _append_diarization_log(
        ...
        try:
            ...
            for i, seg in enumerate(final_transcript):
                ...
                log_lines.append(f"      文本: {seg['transcript'][:100]}...")
            log_lines.append("")
            log_lines.append("=" * 80)
