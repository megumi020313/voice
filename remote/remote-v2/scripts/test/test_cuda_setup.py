...
def test_whisper_cuda():
    """测试 Whisper 模型在 CUDA 上的运行"""
    print("\n" + "="*80)
    print("测试 4: Whisper 模型 CUDA 推理")
    print("="*80)
    
    if not torch.cuda.is_available():
        print("⚠️  CUDA 不可用，跳过 Whisper CUDA 测试")
        return False
    
    try:
        from faster_whisper import WhisperModel
        
        print("正在加载 Whisper tiny 模型（CUDA）...")
...
def main():
    ...
    # 测试4: Whisper CUDA
    results.append(("Whisper CUDA", test_whisper_cuda()))
